...
def extraer_datos_2026():
    ...
    tabla_ind = 'aps_2026_integrantes' if table_exists('aps_2026_integrantes') else 'caracterizacion_si_aps_individual_2026'
        
    if table_exists(tabla_ind):
        data["poblacion"] = get_count(f'SELECT COUNT(*) FROM "{tabla_ind}"')
        
        col_sexo = find_column(tabla_ind, ['sexo', 'sexo_al_nacer', '8_sexo'])
        if col_sexo:
    ...
    tabla_fam = 'aps_2026_familias' if table_exists('aps_2026_familias') else 'caracterizacion_si_aps_familiar_2026'
